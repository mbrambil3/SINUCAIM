...
agent_communication:
  ...
  - agent: "main"
    message: "Correção definitiva da precisão da mira + persistência entre reinstalações. (1) detectCueStick() reescrita com Histograma Angular em vez de PCA: cada pixel vota num bin de 5° ponderado pela distância à branca, pico do histograma = direção do taco. Robusto a halos de bolas vizinhas (bugs que o PCA sofria). Exige contraste >= 1.8x o noise floor e pixel a pelo menos 3R de distância. Bump ballExcludeR 1.3→1.9. (2) Interpolação parabólica sub-bin para precisão angular. (3) Calibração agora espelhada em window.localStorage (LS_KEY='sinucadaAimCalibration_v2'). localStorage é por-domínio e sobrevive à desinstalação da extensão, então ao reinstalar, a calibração de sinucada.com é automaticamente restaurada. Fallback também aplicado no botão 'Carregar calibragem'."
