...
agent_communication:
  ...
  - agent: "main"
    message: "Reescrita da função detectCueStick() com PCA (Principal Component Analysis): (1) janela de busca aumentada para 14R com margem de ~40-60px FORA do polígono (captura o corpo do taco que hangover os rails), (2) filtro de brilho mais rigoroso para pixels externos (r+g+b >= 150 rejeita painel lateral, chat e scoreboard), (3) cálculo de covariância 2x2 single-pass → autovetor principal = eixo real do taco (robusto a noise disperso), (4) check de elongação lambda1/lambda2 >= 2 garante que o cluster é linear (não blob), (5) orientação do eixo pela projeção (cue - CoM) · v garante direção correta do tiro. Também: quando cueStick é null, limpa targetBall auto-selecionado obsoleto (evita mira congelada)."
